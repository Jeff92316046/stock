                    ...
                    if len(temp)==16:
                        for i in range(16):
                            ...
                            if len(temp_1) != 0 and len(temp_2)!=0 :
                                ...
                                print(f"insert || stock : {clean_str(gsc[k])}, date : {clean_str(date_text)}, number : {clean_str(i+1)}")
                            else:
                                temp_3 = driver.find_element('xpath','/html/body/div[1]/div[1]/div/main/div[6]/div/table/tbody/tr/td/span').text
                                if temp_3 == '查無此資料':
                                    print(f"no data || stock : {clean_str(gsc[k])}, date : {clean_str(date_text)}, number : {clean_str(i+1)}")
                                    break
                    elif len(temp) == 17:
                        flag = 0
                        for i in range(15):
                        ...
                        if flag == 0:
                            ...
                            print(f"insert || stock : {clean_str(gsc[k])}, date : {clean_str(date_text)}, number : 16")
